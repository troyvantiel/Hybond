...
#include <sys/time.h>
#include <gpuheader.h>
#include <gpuewrecellp_kernel.cu>
////////////////////////////////////////////////////////////////////////////////
extern "C"
void gpuewrecellp_ 
 ...
 int disp)
{
  ...
  for(i = 0; i < 3; i++){
    ...
    if (num_cell[i] < num_div_cell*2+1)
      printf("ERROR gpuewrecellp : too small domain size \n");
  ...
  for (i = 0; i < total_num_cell; i++){
	...
	printf("ERROR ewrecellp : too small num_max_atm_per_cell \n");
  ...
  if (itmp*num_thread_per_cell > BEWRCELL)
    printf("ERROR gpuewrecellp!!! too much number of round cell num_round_cell:%d num_thread_per_cell:%d\n",num_round_cell,num_thread_per_cell);
  ...
  gpuewrecellp_kernel<<< grid , threads >>>
  ...
  for (i = 0; i < total_num_cell; i++)
    if (cell_size[i] > 0){
      ...
      itmp1 = cell_base[i];
      for (j = 0; j < cell_size[i]; j++){
	COPY_SINGLE_TO_STRESS(f_float,force,itmp1,itmp+j)
	itmp1 = cell_link[itmp1];
      }
    }
