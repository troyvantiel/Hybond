...
#include <sys/time.h>
#include <gpuheader.h>
#include <gpuvdwcutoffdble_kernel.cu>
////////////////////////////////////////////////////////////////////////////////

extern "C"
void gpuvdwcutoffdble_ 
(double*, int, int*, int, double*, double*, int, double*, int, int, double*, double*, int, int, int, double);
void gpuvdwcutoffdble__
(double*, int, int*, int, double*, double*, int, double*, int, int, double*, double*, int, int, int, double);
 ...
 int begin_numi, int num_atmi, double radius_cut)
{
  ...
  CUDA_SAFE_CALL(cudaSetDevice(idevice));
   ...
   unsigned int memsize_f_float = sizeof(float4) * ni * 2;
   ...
   //   for (int i = 0; i < nat+1; i++)
     //     for (int j = 0; j < nat+1; j++){
       //       itmp1 = i*(nat+1)+j;
       //       printf("%d %d %f %f %f\n",i,j,gr_float[itmp1].x,gr_float[itmp1].y,gr_float[itmp1].z);
       //     } 
   ...
   gpuvdwcutoffdble_kernel<<< grid, threads >>>
   ...
   for (int i = 0; i < num_atm_i; ++i){
     ...
     COPY_SINGLE_TO_FORCEPOT_DOUBLE(f_float,force,pot,itmp,i)
   }
#if 0
