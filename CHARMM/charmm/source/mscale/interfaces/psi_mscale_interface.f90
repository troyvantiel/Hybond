...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  program(1:LNWCPATH)='/v/apps/psi-3.3.0/bin/psi3'
  !
  if (argc.eq.0) then
     write(*,*)'No parameters.'
     call exit
...
100 continue
  ...
  CALL getpsi3(NATOM,FOUT(1:ll),QMEL,X,Y,Z)
...
100 continue
  ...
  if(((index(line,'geometry')).gt.0) .or.  &
         ((index(line,'Geometry')).gt.0)   &
         .or. ((index(line,'GEOMETRY')).gt.0)) then
     write(2,*)line
     wrtgeom=.true.
  endif
...
subroutine getpsi3(natom,fname,ener,dx,dy,dz)
  implicit none
  ...
  open(unit=3,file='psi.file11.dat',status='unknown')
  !     open(unit=3,file=rname(1:ll),status='unknown')

  read(3,'(a120)',end=101)line  ! read title 

100 continue
  read(3,'(a120)',end=101)line

  read(line,'(11x,f14.10)')ener
  !write(*,*)ener,ener*tokcal
  ener=ener*tokcal
  ...
  do i=1,natom
     read(3,'(a120)',end=101)line
     !read(line,'(27x,3f13.10)')dx(i),dy(i),dz(i)
     read(line,'(27x,f13.10,7x,f13.10,7x,f13.10)')dx(i),dy(i),dz(i)
     !write(*,'(3f15.10)')dx(i),dy(i),dz(i)
     dx(i)=dx(i)*tokcal/bohrr
     dy(i)=dy(i)*tokcal/bohrr
     dz(i)=dz(i)*tokcal/bohrr
     !write(*,'(3f22.10)')dx(i),dy(i),dz(i)
  enddo


  !     ifound = index(line,'SCF forces in the reference frame')
  !     if(ifound.gt.0) then
  !        read(3,'(a120)',end=101)line
