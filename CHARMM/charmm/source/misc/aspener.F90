...
#endif 
  CALL ASPEN1(EU,X,Y,Z,DX,DY,DZ,QECONT,ECONT,RX,AREA, &
...
#endif 
  RETURN
END SUBROUTINE ASPENR
!
SUBROUTINE ASPEN1(EU,X,Y,Z,DX,DY,DZ,QECONT,ECONT, &
...
17      CONTINUE
        ...
        if (.not.ISKIP(K))then
           ...
           DO L=1,IO
              if (.not.ISKIP(L))then
                 ...
                 if (ACOS( ACOS_INPUT ).lt.THERK+THER(L)) then
                    ...
                    !
                    ! IF((IMERI.EQ.-290).AND.(MYNOD.EQ.7).AND.(IR.EQ.551).AND.(K.EQ.7)) THEN
                    !       WRITE(OUTU,'(A,5I5,2D20.10)')'TK1:',MYNOD,IMERI,IR,K,KARC,TB,TK1
                    ! ENDIF
                    !
                    TB=(TXB-TYR)/TD
                    if (ABS(TB).gt.(ONE-RSMALL)) &
                    ...
                    !
                    ! IF((IMERI.EQ.-290).AND.(MYNOD.EQ.7).AND.(IR.EQ.551).AND.(K.EQ.7))THEN
                    !       WRITE(OUTU,'(A,5I5,2D20.10)')'TK2:',MYNOD,IMERI,IR,K,KARC,TB,TK2
                    ! ENDIF
                    !
                    ...
                    ARCI(KARC)=TI
                    ! IF((IMERI.EQ.-290).AND.(MYNOD.EQ.7).AND.(IR.EQ.551).AND.(K.EQ.7))THEN
                    !       WRITE(OUTU,'(A,5I5,3F10.5)') &
                    !           'ARC:',MYNOD,IMERI,IR,K,KARC,ARCI(KARC),ARCF(KARC)
                    ! ENDIF
                    LT(KARC)=L
                    EX(KARC)=THE
                    UX(L)=UXL
           ...
           if (.not.LTOP) then
              ...
              KOUT(JB_LOCAL)=K*1024+L
           ENDIF
           !**Calculate derivatives. Comments refer to the equivalent notation in
           !  Tim Richmond's paper.
           do L=1,IO
              !                    K ; K index is J
              if (IDER(L).ne.0) then
                 !                                         IDER(L)* rho**2
                 ...
                 !            x component of derivative in mu coordinate system
                 GACB = GK - UZL*GL/B(L)
                 !                                         y component of
                 ...
                 !                  z component of derivative in mu coordinate system
...
4    CONTINUE
  ...
  CALL FIND_ASP(AREA, NATOM)
  RETURN
END SUBROUTINE ASPEN1

!----------------------------------------------------------------------------
SUBROUTINE SURFACE_IO(IUNIT)
  !
  !  This routine reads in solvation input data, from unit 83.
  !  Same as original Surface_Io except that ASP values are 10 times bigger.
  !
  ! vap_to_wat_kd.in  This is the input file with solvation energy data, using
  !             atomic solvation parameters derived from Wolfenden free
  !             energies of transfer, corrected for standard state by Kyte
  !             and Doolittle.
  !
  ! vap_to_wat_honig.in  This is the input file with solvation energy data, using
  !             atomic solvation parameters derived from Wolfenden free
...
#endif 
  !
  SOLVENT_ENTERED=.TRUE.
  DX(1:NATOM)=zero
  DY(1:NATOM)=zero
  DZ(1:NATOM)=zero
  CALL ASPENR(EDUM,X,Y,Z,DX,DY,DZ,.FALSE.,ECONT0)
  !
  RETURN
END SUBROUTINE SURFACE_IO

subroutine Find_Asp(AREA, NATOMX)
  !
