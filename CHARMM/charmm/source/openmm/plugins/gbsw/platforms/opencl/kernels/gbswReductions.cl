...
__kernel void reduceBornSum(int bufferSize, int numBuffers, float alpha, float beta, float gamma,
...
#endif
            __global const float2* restrict params, __global real* restrict bornRadii, __global real* restrict gbswChain) {
    ...
    while (index < NUM_ATOMS) {
        ...
        // Now calculate Born radius and GBSW term.
...
#endif
            __global real* restrict energyBuffer, __global const float2* restrict params, __global const real* restrict bornRadii, __global const real* restrict gbswChain) {
    real energy = 0.0f;
    unsigned int index = get_global_id(0);
    while (index < NUM_ATOMS) {
        // Sum the Born force
        ...
        energy += saTerm;
        force *= bornRadius*bornRadius*gbswChain[index];
